...
interface DashboardChartsProps {
  ...
  chartDataA?: ChartDataPoint[];
  chartDataB?: ChartDataPoint[];
  overlay?: boolean;
}

const DashboardCharts: React.FC<DashboardChartsProps> = ({ chartData = [], chartDataA, chartDataB, overlay }) => {
  // Overlay mode: show both datasets on the same chart
  if (overlay && chartDataA && chartDataB) {
    return (
      <Box>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          Overlay: Red = Report A, Blue/Green = Report B
        </Typography>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart margin={{ top: 16, right: 32, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#333" />
            <XAxis dataKey="name" stroke="#fff" />
            <YAxis stroke="#fff" />
            <Tooltip />
            <Legend />
            <Bar dataKey="requests" data={chartDataA} fill="#e53935" name="Requests (A)" />
            <Bar dataKey="requests" data={chartDataB} fill="#1e88e5" name="Requests (B)" />
            <Bar dataKey="responses" data={chartDataA} fill="#43a047" name="Responses (A)" />
            <Bar dataKey="responses" data={chartDataB} fill="#fbc02d" name="Responses (B)" />
          </BarChart>
        </ResponsiveContainer>
        <Box mt={4}>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart margin={{ top: 16, right: 32, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#333" />
              <XAxis dataKey="name" stroke="#fff" />
              <YAxis stroke="#fff" />
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="p50" data={chartDataA} stroke="#e53935" name="p50 (A)" strokeWidth={2} />
              <Line type="monotone" dataKey="p50" data={chartDataB} stroke="#1e88e5" name="p50 (B)" strokeWidth={2} />
              <Line type="monotone" dataKey="p90" data={chartDataA} stroke="#ffb300" name="p90 (A)" strokeWidth={2} />
              <Line type="monotone" dataKey="p90" data={chartDataB} stroke="#43a047" name="p90 (B)" strokeWidth={2} />
              <Line type="monotone" dataKey="p99" data={chartDataA} stroke="#8e24aa" name="p99 (A)" strokeWidth={2} />
              <Line type="monotone" dataKey="p99" data={chartDataB} stroke="#00bcd4" name="p99 (B)" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </Box>
      </Box>
